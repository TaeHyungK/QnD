...
import org.sopt21.qnd.LockScreen.ScreenService
...
//        lockInfo = activity.getSharedPreferences("lockInfo", 0)
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            ...
            R.id.switch_lock -> {
                /*switch_lock_checked = switch_lock.isChecked
                var editor: SharedPreferences.Editor = lockInfo.edit()
                editor.putBoolean("check_lock", switch_lock_checked!!)
                editor.commit()*/
                if (switch_lock_checked as Boolean) {
